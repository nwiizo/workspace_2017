...
fn main() {
    println!("`mkdir sample`");
    // ディレクトリを作成する。返り値は`io::Result<()>`
    match fs::create_dir("sample") {
        Err(why) => println!("! {:?}", why.kind()),
        Ok(_) => {},
    }

    println!("`echo hello > sample/hello.txt`");
    // 上のmatchは`unwrap_or_else`をメソッドを用いて簡略化できる。
    echo("hello", &Path::new("sample/hello.txt")).unwrap_or_else(|why| {
        println!("! {:?}", why.kind());
    });

    println!("`mkdir -p i/kill/you`");
    // 再帰的にディレクトリを作成する。返り値は`io::Result<()>`
    fs::create_dir_all("i/kill/you").unwrap_or_else(|why| {
        println!("! {:?}", why.kind());
    });

    println!("`touch i/kill/tom.txt`");
    touch(&Path::new("i/kill/tom.txt")).unwrap_or_else(|why| {
        println!("! {:?}", why.kind());
    });
